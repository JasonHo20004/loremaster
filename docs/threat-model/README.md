...
| --- | --- | --- |
| Answer or future evidence in JSON, source map, error, cache or telemetry | Explicit ACTIVE projection; server-only content; no body/query/SQL-parameter logging; no shared cache of private HTTP responses | S4 answer-absence assertions; S5 errors/logs; S6/S8 bundle scan; S12 two-guest cache isolation |
...
| XSS through names or imported lore | Plain text rendering, bounded strings, CSP and secure headers; server pseudonyms | S5 import validation; S6 malicious text renders inert |
